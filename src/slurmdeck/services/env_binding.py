...
def _conda_env_var_loader(prefix: str, python_executable: str, *, indent: str = "") -> str:
    command = f"{shlex.quote(python_executable)} -c {shlex.quote(_CONDA_ENV_VARS_LOADER)} {shlex.quote(prefix)}"
    return f"{indent}_slurmdeck_conda_env_vars=$({command})"
    ...
    *,
    ...
    lines.extend(
        [
            ...
            _conda_env_var_loader(binding.prefix, agent_python, indent="  "),
            f"  export CONDA_PREFIX={quoted}",
            "  export PATH={}:$PATH".format(shlex.quote(binding.prefix + "/bin")),
            '  eval "$_slurmdeck_conda_env_vars"',
            "  unset _slurmdeck_conda_env_vars",
        ]
    )
    lines.extend(
        [
