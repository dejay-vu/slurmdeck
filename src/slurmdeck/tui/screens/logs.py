"""One task's log: bounded tail plus optional follow mode."""
...
class LogsScreen(DeckScreen):
    ...
    def reload(self) -> None:
        # broadcast data refreshes must not re-fetch logs over ssh
        self._update_title()
    ...
    def _update_title(self, note: str = "") -> None:
        text = Text()
        text.append(f" {self.run_id} / task {self.task_id}", style="bold")
        text.append(f" · std{self.stream}", style="ui.stdout" if self.stream == "out" else "ui.stderr")
        ...
        text.append(f"   (last {TAIL_LINES} lines)", style="dim")
    ...
    def action_switch_stream(self) -> None:
        self._stop_follow()
        self.stream = "err" if self.stream == "out" else "out"
        self._content = ""
        self._render_content()
        self._fetch()
    ...
    def action_toggle_follow(self) -> None:
        ...
        try:
            self._handle = self.controller.follow_log(self.run_id, self.task_id, stream=self.stream, on_line=on_line)
